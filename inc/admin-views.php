...
function eioftx_staff_member_info_meta_box(){
	global $post;
...
function eioftx_staff_member_warning_meta_box() {
...
function eioftx_staff_member_bio_meta_box(){
	global $post;
	$custom = get_post_custom($post->ID);
	...
	<p class="eioftx-note">**Note: HTML is allowed.</p>
	
	<?php wp_nonce_field('eioftx_post_nonce', 'eioftx_add_edit_staff_member_noncename') ?>
...
add_action( "manage_posts_custom_column", "eioftx_staff_member_display_custom_columns");

function eioftx_staff_member_display_custom_columns( $column ) {
  global $post;
...
function eioftx_staff_member_order_page() {
?>
	<div class="wrap">
...
function eioftx_staff_member_usage_page() {
	
	$output .= '<div class="wrap eioftx-usage">';
	...
	$output .= '<table><tbody>';
	$output .= '<tr><td width="280px"><code>[eioftx-staff-list]</code></td><td>This is the most basic usage of Simple Staff List. Displays all Staff Members on post or page.</td></tr>';
	$output .= '<tr><td><code>[eioftx-staff-list <strong>group="Robots"</strong>]</code></td><td>This displays all Staff Members from the group "Robots" sorted by order on the "Order" page. This will also add a class of "Robots" to the outer Staff List container for styling purposes.</td></tr>';
	$output .= '<tr><td><code>[eioftx-staff-list <strong>wrap_class="clearfix"</strong>]</code></td><td>This adds a class to the inner Staff Member wrap.</td></tr>';
	$output .= '<tr><td><code>[eioftx-staff-list <strong>order="ASC"</strong>]</code></td><td>This displays Staff Members sorted by ascending or descending order according to the "Order" page. You may use "ASC" or "DESC" but the default is "ASC"</td></tr>';
	
	$output .= '</tbody></table>';
	
	$output .= '<p>To display your Staff List just use the shortcode <code>[eioftx-staff-list]</code> in any page or post. This will output all staff members according to the template options set <a href="edit.php?post_type=staff-member&page=staff-member-template" title="Edit the Simple Staff List template.">here</a>.</p>';
...
function eioftx_staff_member_template_page(){

	// Get options for default HTML CSS
	$default_html 					= get_option('_staff_listing_default_html');
	$defaultcss 					= get_option('_staff_listing_defaultcss');
    ...
    $write_externalcss				= get_option('_staff_listing_write_externalcss');
    
    $default_tag_ul  = '<ul class="eioftx-tag-list">';
    ...
    $default_formatted_tag_ul  = '<ul class="eioftx-tag-list">';
	...
	if ( !empty($_POST) && check_admin_referer( 'staff-member-template', 'staff-list-template' ) ) {
		...
		update_option('_staff_listing_customcss', $_POST[ "staff-listing-css"]);
		...
		if ( $_POST[ "write-external-css" ] != "yes" ) {
			update_option('_staff_listing_write_externalcss', "no");
			$write_externalcss = "no";
		} else {
			update_option('_staff_listing_write_externalcss', $_POST[ "write-external-css" ]);
			$write_externalcss = "yes";
		...
		if ( $write_externalcss == "yes" ) {
			...
			if (file_exists($filename)){
				...
				$customcss  = stripslashes_deep(get_option('_staff_listing_defaultcss'));
				update_option('_staff_listing_customcss', $customcss);
				file_put_contents($filename, $customcss);
			}
		} else {
	...
	if ( $write_externalcss == 'yes' ){
		$extcss_check = "checked";
	}
	
	$output .= '<div class="wrap eioftx-template">';
	$output .= '<div id="icon-edit" class="icon32 icon32-posts-staff-member"><br></div><h2>Simple Staff List</h2>';
    ...
    $output .= '<p><input type="checkbox" name="write-external-css" id="write-external-css" value="yes" '.$extcss_check.' /><label for="write-external-css"> Write to external CSS file? (Leave unchecked for WP Multisite.)</label>';
    
    $output .= '<div class="default-css">
    				<h4 class="heading button-secondary">View Default CSS</h4>
    				<div class="content">
    					<pre>'.htmlspecialchars(stripslashes_deep($defaultcss)).'</pre>
    				</div>
    			</div><br />';
    ...
    $output .= '<textarea name="staff-listing-css" cols="120" rows="16">'.$customcss.'</textarea>';
