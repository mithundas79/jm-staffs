...
/////	
		...
		add_action( 'wp_ajax_staff_member_update_post_order', 'eioftx_staff_member_update_post_order' );
		
		function eioftx_staff_member_update_post_order() {
			global $wpdb;
...
//////
		...
		add_action('save_post', 'eioftx_save_staff_member_details');
		
		function eioftx_save_staff_member_details(){
			global $post;
			
			if ( !isset( $_POST['eioftx_add_edit_staff_member_noncename'] ) || !wp_verify_nonce( $_POST['eioftx_add_edit_staff_member_noncename'], 'eioftx_post_nonce' ) ) {
				return;
			}
